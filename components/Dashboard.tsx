  ...
  return (
    <div className="px-4 md:px-8 max-w-7xl mx-auto animate-fade-in">
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white mb-1">Hi, {profile.name.split(' ')[0]}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-4">
           <div className="flex lg:flex-col overflow-x-auto pb-4 lg:pb-0 gap-3 lg:gap-3 no-scrollbar">
             {days.map((day, idx) => {
               ...
               return (
                 <GlassCard 
                    ...
                    className={`
                      cursor-pointer transition-all duration-200 min-w-[80px] lg:min-w-0 group
                      ${isActive ? 'ring-1 ring-purple-500 bg-white/15 transform scale-[1.02]' : 'hover:bg-white/10'}
                    `}
                 >
                   <div className="p-3 flex flex-col lg:flex-row items-center lg:justify-between gap-2">
                      <div className="text-center lg:text-left">
                        <div className={`text-xs font-bold uppercase tracking-wider mb-1 ${isActive ? 'text-purple-300' : 'text-white/40'}`}>{dayDisplay}</div>
                        <div className="text-white font-medium text-xs lg:text-sm truncate max-w-[80px] lg:max-w-[150px] hidden lg:block">
                            {dayPlan?.focus || "Rest"}
                        </div>
                        <div className={`w-1.5 h-1.5 rounded-full mx-auto lg:hidden ${isRest ? 'bg-white/10' : isActive ? 'bg-purple-400' : 'bg-white/40'}`} />
                      </div>
                      ...
                      {!isRest && (
                        <div className="hidden lg:block">
                            {completion === 100 ? (
                                ...
                                <div className="text-xs text-white/40 font-mono">{completion}%</div>
                            )}
                        </div>
                      )}
                   </div>
        ...
        <div className="lg:col-span-8">
          ...
          {!activeDayPlan.exercises.length ? (
            <GlassCard className="p-12 flex flex-col items-center justify-center text-center min-h-[400px]">
              ...
              <p className="text-white/60 max-w-md text-sm mb-8">
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                  <GlassCard className="p-4 flex items-center gap-3">
                      <div className="p-2 rounded-lg bg-blue-500/20 text-blue-300"><Clock className="w-5 h-5"/></div>
                      <div>
                          <div className="text-xs text-white/50 uppercase">Duration</div>
                          <div className="text-white font-bold">{estDuration} min</div>
                      </div>
                  </GlassCard>
                  <GlassCard className="p-4 flex items-center gap-3">
                      <div className="p-2 rounded-lg bg-purple-500/20 text-purple-300"><Layers className="w-5 h-5"/></div>
                      <div>
                          <div className="text-xs text-white/50 uppercase">Volume</div>
                          <div className="text-white font-bold">{totalSets} Sets</div>
                      </div>
                  </GlassCard>
                   <GlassCard className="p-4 flex items-center gap-3 hidden md:flex">
                      <div className="p-2 rounded-lg bg-pink-500/20 text-pink-300"><Activity className="w-5 h-5"/></div>
                      <div>
                          <div className="text-xs text-white/50 uppercase">Focus</div>
                          <div className="text-white font-bold truncate max-w-[100px]">{activeDayPlan.focus}</div>
                      </div>
                  </GlassCard>
              </div>
                ...
                {activeDayPlan.exercises.map((exercise, idx) => (
                  <GlassCard 
                    ...
                    className={`group transition-all duration-300 cursor-pointer relative overflow-hidden
                      ${exercise.isCompleted ? 'opacity-60 bg-black/20' : 'hover:bg-white/15 hover:scale-[1.01] hover:shadow-xl hover:shadow-purple-500/10'}
                    `}
                  >
                    <div className="p-5">
                       ...
                       <div className="flex items-start gap-4 mb-4">
                            <button 
                                ...
                                ${exercise.isCompleted 
                                    ...
                                    : 'border-white/20 hover:border-purple-400 bg-transparent hover:scale-110'
                                }`}
                            >
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between items-start">
                                    <h3 className={`text-lg font-bold leading-tight ${exercise.isCompleted ? 'text-white/50 line-through' : 'text-white'}`}>
                                        {exercise.name}
                                    </h3>
                       ...
                       <div className="grid grid-cols-4 gap-2 bg-black/20 rounded-xl p-3 border border-white/5">
                           <div className="text-center border-r border-white/10">
                               <div className="text-[10px] text-white/40 uppercase mb-1">Sets</div>
                               <div className="text-white font-mono font-semibold">{exercise.sets}</div>
                           </div>
                           <div className="text-center border-r border-white/10">
                               <div className="text-[10px] text-white/40 uppercase mb-1">Reps</div>
                               <div className="text-white font-mono font-semibold">{exercise.reps}</div>
                           </div>
                           <div className="text-center border-r border-white/10">
                               <div className="text-[10px] text-white/40 uppercase mb-1">Weight</div>
                               <div className="text-white font-mono font-semibold truncate px-1">{exercise.weight || '-'}</div>
                           </div>
                            <div className="text-center">
                               <div className="text-[10px] text-white/40 uppercase mb-1">Rest</div>
                               <div className="text-white font-mono font-semibold text-pink-300">{exercise.rest || '60s'}</div>
                           </div>
                       </div>

                       {/* Footer: Notes */}
                       {exercise.notes && (
                           <div className="mt-3 text-xs text-white/40 flex items-center gap-2 italic">
